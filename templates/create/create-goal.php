 ...
 *
 * Combines create and edit functionality for a skater's goal,
...
$goal_id = get_query_var( 'edit_goal' );
$is_edit = ! empty( $goal_id ) && get_post_type( $goal_id ) === 'goal';

// Set the post_id for the form.
$post_id = $is_edit ? intval( $goal_id ) : 'new_post';
...
$page_title  = $is_edit ? 'Update Goal' : 'Create New Goal';
$submit_text = $is_edit ? 'Update Goal' : 'Create Goal';


// --- ACF FORM SETTINGS ---

$goal_form_settings = array(
    'post_id'         => $post_id,
    'post_title'      => true, // Goal CPT uses the post title field.
    'post_content'    => false,
    'field_groups'    => array( 'group_681c4115a026f' ), // Field group for 'Goal' from your JSON file.
    'new_post'        => array(
        'post_type'   => 'goal',
        'post_status' => 'publish',
    ),
    ...
    'uploader'        => 'wp',
    'return'          => $return_url,
    'updated_message' => __( 'Goal saved successfully.', 'wp-coach' ),
);

// Pre-populate the skater field if creating a new goal from a skater's page.
