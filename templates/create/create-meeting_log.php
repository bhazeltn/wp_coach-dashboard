 ...
 *
 * Combines create and edit functionality for a meeting log entry,
...
// Check if we are editing an existing meeting log or creating a new one.
$meeting_log_id = get_query_var( 'edit_meeting_log' );
$is_edit        = ! empty( $meeting_log_id ) && get_post_type( $meeting_log_id ) === 'meeting_log';

// Set the post_id for the form.
$post_id = $is_edit ? intval( $meeting_log_id ) : 'new_post';

// Determine the skater ID for the return URL and for pre-populating the form.
$skater_id_for_return = null;
if ( $is_edit ) {
    $skater_posts = get_field( 'skater', $post_id ); // This field can have multiple skaters
    if ( ! empty( $skater_posts ) ) {
        // Just use the first skater for the return URL if multiple are selected.
        $first_skater = $skater_posts[0];
        $skater_id_for_return = $first_skater->ID;
    }
...
$page_title  = $is_edit ? 'Update Meeting Log' : 'Create New Meeting Log';
$submit_text = $is_edit ? 'Update Log' : 'Create Log';


// --- ACF FORM SETTINGS ---

$meeting_log_form_settings = array(
    'post_id'         => $post_id,
    'post_title'      => false, // Meeting Log CPT does not use the title.
    'post_content'    => false,
    'field_groups'    => array( 'group_6824296728a7d' ), // Field group for 'Meeting Details' from your JSON file.
    'new_post'        => array(
    ...
    'uploader'        => 'wp',
    'return'          => $return_url,
    'updated_message' => __( 'Meeting log saved successfully.', 'wp-coach' ),
);
...
if ( $skater_id_for_return && ! $is_edit ) {
    add_filter( 'acf/load_value/name=skater', function( $value ) use ( $skater_id_for_return ) {
        if ( ! $value ) {
            // The skater field can be a relationship, so we pass the ID in an array.
            $value = array( $skater_id_for_return );
        }
        return $value;
    }, 10, 1 );
}
...
<main class="w-full bg-gray-100 min-h-screen">
    <div class="container mx-auto p-4 md:p-8">
        ...
        <div class="w-full bg-white rounded-lg shadow-lg p-6 md:p-8">
            <?php 
            // Render the ACF form.
            acf_form( $meeting_log_form_settings ); 
            ?>
        </div>
