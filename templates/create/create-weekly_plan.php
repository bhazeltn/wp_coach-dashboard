 ...
 *
 * Combines create and edit functionality for a weekly plan,
...
// Check if we are editing an existing weekly plan or creating a new one.
$weekly_plan_id = get_query_var( 'edit_weekly_plan' );
$is_edit        = ! empty( $weekly_plan_id ) && get_post_type( $weekly_plan_id ) === 'weekly_plan';

// Set the post_id for the form.
$post_id = $is_edit ? intval( $weekly_plan_id ) : 'new_post';
...
$page_title  = $is_edit ? 'Update Weekly Plan' : 'Create New Weekly Plan';
$submit_text = $is_edit ? 'Update Plan' : 'Create Plan';


// --- ACF FORM SETTINGS ---

$weekly_plan_form_settings = array(
    'post_id'         => $post_id,
    'post_title'      => false, // Weekly Plan CPT does not use the title.
    'post_content'    => false,
    'field_groups'    => array( 'group_681c3d5d75241' ), // Field group for 'Weekly Plan Details' from your JSON file.
    'new_post'        => array(
    ...
    'uploader'        => 'wp',
    'return'          => $return_url,
    'updated_message' => __( 'Weekly plan saved successfully.', 'wp-coach' ),
);
...
<main class="w-full bg-gray-100 min-h-screen">
    <div class="container mx-auto p-4 md:p-8">
        ...
        <div class="w-full bg-white rounded-lg shadow-lg p-6 md:p-8">
            <?php 
            // Render the ACF form.
            acf_form( $weekly_plan_form_settings ); 
            ?>
        </div>
