<?php
/**
 * Template: Create or Edit Program
 *
 * Combines create and edit functionality for a skater's program,
...
// Check if we are editing an existing program or creating a new one.
$program_id = get_query_var( 'edit_program' );
$is_edit    = ! empty( $program_id ) && get_post_type( $program_id ) === 'program';

// Set the post_id for the form.
$post_id = $is_edit ? intval( $program_id ) : 'new_post';
...
$page_title  = $is_edit ? 'Update Program' : 'Create New Program';
$submit_text = $is_edit ? 'Update Program' : 'Create Program';


// --- ACF FORM SETTINGS ---

$program_form_settings = array(
    'post_id'         => $post_id,
    'post_title'      => true, // Program CPT uses the post title (e.g., "Short Program").
    'post_content'    => false,
    'field_groups'    => array( 'group_682426973af85' ), // Field group for 'Program Details' from your JSON file.
    'new_post'        => array(
        'post_type'   => 'program',
        'post_status' => 'publish',
    ),
    ...
    'uploader'        => 'wp',
    'return'          => $return_url,
    'updated_message' => __( 'Program saved successfully.', 'wp-coach' ),
);

// Pre-populate the skater field if creating a new program from a skater's page.
...
<main class="w-full bg-gray-100 min-h-screen">
    <div class="container mx-auto p-4 md:p-8">
        ...
        <div class="w-full bg-white rounded-lg shadow-lg p-6 md:p-8">
            <?php 
            // Render the ACF form.
            acf_form( $program_form_settings ); 
            ?>
        </div>
