 ...
 *
 * Combines create and edit functionality for a gap analysis,
...
// Check if we are editing an existing gap analysis or creating a new one.
$gap_analysis_id = get_query_var( 'edit_gap_analysis' );
$is_edit         = ! empty( $gap_analysis_id ) && get_post_type( $gap_analysis_id ) === 'gap_analysis';

// Set the post_id for the form.
$post_id = $is_edit ? intval( $gap_analysis_id ) : 'new_post';
...
if ( $is_edit ) {
    ...
    if ( $skater_post ) { // Skater field returns an ID for this CPT
        $skater_id_for_return = $skater_post;
    }
...
$page_title  = $is_edit ? 'Update Gap Analysis' : 'Create New Gap Analysis';
$submit_text = $is_edit ? 'Update Gap Analysis' : 'Create Gap Analysis';


// --- ACF FORM SETTINGS ---

$gap_analysis_form_settings = array(
    'post_id'         => $post_id,
    'post_title'      => true, // Gap Analysis has a title (e.g., "2024-2025 Season Analysis")
    'post_content'    => false,
    'field_groups'    => array( 'group_gap_analysis' ), // Field group from your JSON file.
    'new_post'        => array(
    ...
    'uploader'        => 'wp',
    'return'          => $return_url,
    'updated_message' => __( 'Gap Analysis saved successfully.', 'wp-coach' ),
);

// Pre-populate the skater field if creating a new analysis from a skater's page.
...
<main class="w-full bg-gray-100 min-h-screen">
    <div class="container mx-auto p-4 md:p-8">
        ...
        <div class="w-full bg-white rounded-lg shadow-lg p-6 md:p-8">
            <?php 
            // Render the ACF form.
            acf_form( $gap_analysis_form_settings ); 
            ?>
        </div>
